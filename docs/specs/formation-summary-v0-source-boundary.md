...
|---|---|
| Are kneels excluded? | **Yes.** `play_type == "qb_kneel"` rows are excluded from formation/tendency rates, following the already-proven `team_week_raw_v0` policy (`docs/data/team-week-raw-v0-2024-pr-c-preflight.md` §2, "Kneels"; `scripts/build_team_week_raw_v0_2024_candidate.py::is_competitive_play`) — kneels are not formation-tendency plays. |
| Are spikes excluded? | **Yes**, same rule and same precedent as kneels (§2, "Spikes"). |
...
- Live `nflreadpy.load_pbp(seasons=[2024])` fetch attempted **in this session** (distinct from the `team_week_raw_v0` build above, which ran successfully in a prior session/environment), blocked by this session's egress policy (403 on the nflverse-data release asset host) — reported per environment guidance, not retried.
