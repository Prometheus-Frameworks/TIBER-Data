...
| Command attempted | `nflreadpy.load_pbp(seasons=[2024])` — the exact call the issue recommends, matching `src/team_state/loader.py`'s production usage and `scripts/build_team_week_raw_v0_2024_candidate.py`'s prior successful real-data build |
...
This is a session/environment-level egress policy characteristic, not evidence about the nflverse data itself, and not a claim that TIBER-Data's real execution environment has this same restriction — `src/team_state/loader.py` and `scripts/build_team_week_raw_v0_2024_candidate.py` both depend on this exact call succeeding elsewhere, and the latter has a real, committed, validated 2024 artifact proving it did succeed at least once (`exports/candidates/team_week_raw/team_week_raw_v0_2024_real_source_candidate.json`).
...
**A future verification attempt should run in an environment where `github.com` is reachable** — e.g., this repo's actual CI/development environment, where `src/team_state/loader.py` and `scripts/build_team_week_raw_v0_2024_candidate.py` are already known to reach `nflreadpy.load_pbp()` successfully (the latter's real, committed 2024 candidate artifact is direct proof of at least one successful run). This sandbox is not that environment, twice now, across two independent sessions.
...
- `exports/candidates/team_week_raw/team_week_raw_v0_2024_real_source_candidate.json` (544 rows, `partial_real_data`, `allPassed: true`) remains the only real, committed, validated evidence in this repo of a successful `nflreadpy.load_pbp([2024])` call — produced in a different session/environment than this one.
