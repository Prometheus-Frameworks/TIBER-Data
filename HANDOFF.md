...
## Current handoff — #218 network rerun: 2015-2020 source availability observed (supersedes the #216/#217 blocked-run state above)

- Active task: TIBER-Data #218 — rerun the accepted #217 audit script, byte-identical (blob `529f832b050dba494146f4b133c202072f587c0e`), from a network-capable environment and commit the regenerated evidence reports (PR #219). Executed locally by the human operator (Windows win32, Python 3.14.0, nflreadpy 0.1.5); independently verified in-session (branch ancestry, blob identity, diff surface, report integrity, full validations).
- Files touched: `docs/reports/player-season-coverage-v0-2015-2020-source-availability.json`, `docs/reports/player-season-coverage-v0-2015-2020-source-availability.md`, `HANDOFF.md` (this note). No script, test, data, export, schema, contract, builder, or Forecast file changed.
- Audit-trigger status: this change *is* the completed audit rerun; independent Codex review of PR #219 raised only the handoff-sync item resolved by this note. Independent human review of PR #219 still required before merge.
- What is now true: all six REG seasons 2015-2020 were observed from the approved source family. Every season passes all seven statuses (`source_rows_exist`, `required_columns_present`, `identity_join_sufficient`, `schedule_method_observed`, `existing_schema_compatible`, `existing_validator_semantics_compatible`, `builder_compatible`): contiguous REG weeks exactly 1-17 per season, zero gaps, zero wrong-season rows, zero duplicate grains, 100% gsis identity joins. `builder_compatible` is `compatible_with_explicit_week_span_parameter` for all six — the narrowest evidence-backed schedule rule is `EXPECTED_REG_WEEKS = set(range(1, 18))` for these seasons only, never the 2021+ `range(1, 19)`. Terminal decision: `may_open_player_season_coverage_2015_2020_candidate_build_issue`.
- What is still missing: a separately authorized and reviewed 2015-2020 candidate-build issue (the decision only permits PROPOSING one); an explicit per-window week-span parameter and a restated `full_season` coverage threshold for 17-week seasons in any future builder; the licensed historical ADP artifact (separate unresolved dependency, untouched).
- What must not be assumed: no 2015-2020 candidate is built, validated, reviewed, or promoted; 2021-2025 REG remains the only promoted player-season window; the phrase "2015-2025 is available" remains prohibited; no Forecast mirror/rerun/validation, ADP ingestion, ranking, or product behavior is authorized by this evidence.
