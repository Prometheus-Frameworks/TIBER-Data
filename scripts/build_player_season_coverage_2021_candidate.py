"""Build the bounded player_season_coverage_v0 CANDIDATE extension for 2021 only.

TIBER-Data #200 (authorized follow-up to #198 / PR #199, which found 2021 REG
QB/RB/WR/TE source-available and builder-compatible via
docs/reports/player-season-coverage-v0-2021-source-availability.{md,json}).

Reuses the same builder semantics as scripts/build_player_season_coverage_2022_2025.py
(the promoted 2022-2025 builder): same column-candidate resolution, same team-context
derivation from week-level team-of-record, same coverage_status rule, same age/career
derivation. This script is intentionally a separate, self-contained file rather than an
edit to the 2022-2025 builder: the promoted 2022-2025 artifact's sha is pinned to that
script's exact current form (see exports/promoted/nfl/PLAYER_SEASON_COVERAGE_V0_PROMOTION_MANIFEST.json),
and this issue does not authorize touching it.

- seasons: 2021 only
- season_type: REG only
- positions: QB, RB, WR, TE
- sources: nflreadpy.load_player_stats (week + reg summary levels) and
  nflreadpy.load_players() -- the same approved source family inspected in #198.

Fails closed (raises, writes nothing) on any of:
- a required source column missing from either summary level;
- zero REG week-level rows found;
- zero REG-summary rows for the included positions;
- one or more of QB/RB/WR/TE having zero rows;
- the aggregate observed REG week span is not the complete {1..18} set (no gaps),
  matching the 2021+ 17-game/18-week coverage_status methodology validated in #198;
- a duplicate (player_id, season, season_type) grain pair;
- the load_players() identity join rate falling below the same floor used in #198
  (0.95);
- the resolved output path landing under exports/promoted/ (defense in depth -- this
  script must never write a promoted artifact).
...
EXPECTED_REG_WEEKS = set(range(1, 19))  # 2021+ 17-game / 18-week season, per #198 methodology check
IDENTITY_JOIN_RATE_FLOOR = 0.95  # matches the floor validated in #198's source-availability check

OUTPUT_PATH = REPO_ROOT / "data/processed/evidence/player_season_coverage_2021_candidate.source_backed.json"
...
def build_candidate_payload(week_df, reg_df, players_df) -> dict:
    """Pure assembly: takes already-loaded source frames, returns the candidate payload.

    Raises SourceFeasibilityError (fail closed, writes nothing) if any required
    compatibility check fails. Never returns a payload with fewer than all four
    positions represented, a duplicate grain, an incomplete REG week span, or an
    identity join rate below the floor.
    """
    ...
    if not week_records:
        raise SourceFeasibilityError(
            f"No REG week-level rows found for season(s) {SEASONS}. Source unavailable or schema changed; "
            "build fails closed rather than emitting an empty/partial candidate."
        )

    observed_weeks = {int(r[wk_week_col]) for r in week_records if r.get(wk_week_col) is not None}
    if observed_weeks != EXPECTED_REG_WEEKS:
        missing = sorted(EXPECTED_REG_WEEKS - observed_weeks)
        extra = sorted(observed_weeks - EXPECTED_REG_WEEKS)
        raise SourceFeasibilityError(
            "Aggregate REG week span is not the complete 1-18 set required by the 2021+ "
            f"coverage_status methodology (see #198). missing_weeks={missing} extra_weeks={extra}. "
            "Build fails closed rather than silently overclaiming full-season coverage."
        )

    team_context = _build_team_context(week_records, wk_season_col, wk_week_col, wk_team_col)

    reg_df_columns = set(reg_df.columns)
    player_id_col = _resolve_column(reg_df_columns, ["player_id", "gsis_id"], "player_id")
    player_name_col = _resolve_column(reg_df_columns, ["player_display_name", "player_name"], "player_name")
    position_col = _resolve_column(reg_df_columns, ["position"], "position")
    games_col = _resolve_column(reg_df_columns, ["games"], "games")
    season_type_col = _resolve_column(reg_df_columns, ["season_type"], "season_type", required=False)

    completions_col = _resolve_column(reg_df_columns, ["completions"], "completions", required=False)
    attempts_col = _resolve_column(reg_df_columns, ["attempts"], "attempts", required=False)
    passing_yards_col = _resolve_column(reg_df_columns, ["passing_yards"], "passing_yards", required=False)
    passing_tds_col = _resolve_column(reg_df_columns, ["passing_tds"], "passing_tds", required=False)
    passing_int_col = _resolve_column(reg_df_columns, ["passing_interceptions"], "passing_interceptions", required=False)
    carries_col = _resolve_column(reg_df_columns, ["carries"], "carries", required=False)
    rushing_yards_col = _resolve_column(reg_df_columns, ["rushing_yards"], "rushing_yards", required=False)
    rushing_tds_col = _resolve_column(reg_df_columns, ["rushing_tds"], "rushing_tds", required=False)
    receptions_col = _resolve_column(reg_df_columns, ["receptions"], "receptions", required=False)
    targets_col = _resolve_column(reg_df_columns, ["targets"], "targets", required=False)
    receiving_yards_col = _resolve_column(reg_df_columns, ["receiving_yards"], "receiving_yards", required=False)
    receiving_tds_col = _resolve_column(reg_df_columns, ["receiving_tds"], "receiving_tds", required=False)
    receiving_air_yards_col = _resolve_column(reg_df_columns, ["receiving_air_yards"], "receiving_air_yards", required=False)
    target_share_col = _resolve_column(reg_df_columns, ["target_share"], "target_share", required=False)
    air_yards_share_col = _resolve_column(reg_df_columns, ["air_yards_share"], "air_yards_share", required=False)
    wopr_col = _resolve_column(reg_df_columns, ["wopr"], "wopr", required=False)
    racr_col = _resolve_column(reg_df_columns, ["racr"], "racr", required=False)
    fantasy_ppr_col = _resolve_column(reg_df_columns, ["fantasy_points_ppr"], "fantasy_points_ppr", required=False)
    ...
    for season in SEASONS:
        for row in _records_from_dataframe(reg_df):
            ...
            if grain_key in seen_grain:
                raise SourceFeasibilityError(f"Duplicate player-season-season_type grain detected ({grain_key}). Build fails closed.")
    ...
    if not records:
        raise SourceFeasibilityError(
            "No valid source-backed player-season coverage records were produced for the included "
            "positions. Build fails closed."
        )

    row_counts_by_position: dict[str, int] = {p: 0 for p in INCLUDED_POSITIONS}
    for r in records:
        row_counts_by_position[r["position"]] += 1
    missing_positions = sorted(p for p in INCLUDED_POSITIONS if row_counts_by_position[p] == 0)
    if missing_positions:
        raise SourceFeasibilityError(
            f"No rows produced for required position(s) {missing_positions}. Build fails closed rather "
            "than emitting a partial-position candidate."
        )

    identity_verified = sum(1 for r in records if r["identity_confidence"] == "source_verified")
    identity_join_rate = identity_verified / len(records)
    if identity_join_rate < IDENTITY_JOIN_RATE_FLOOR:
        raise SourceFeasibilityError(
            f"load_players() identity join rate {identity_join_rate:.4f} is below the required floor "
            f"{IDENTITY_JOIN_RATE_FLOOR}. Build fails closed rather than emitting a candidate with "
            "degraded identity provenance."
        )
    ...
    return {
        ...
        "generated_from": [
            "scripts/build_player_season_coverage_2021_candidate.py",
        ...
        "tracking_issue": "TIBER-Data#200 (follow-up to #198 / PR #199)",
        "non_goals": [
            ...
            "does not modify or merge into the promoted 2022-2025 player_season_coverage_v0 artifact",
        ],
        "methodology": {
        ...
        "identity_join_rate": round(identity_join_rate, 4),
...
def main() -> int:
    ...
    if resolved_output == resolved_promoted_dir or resolved_promoted_dir in resolved_output.parents:
        raise SourceFeasibilityError(
            f"Refusing to write candidate output under a promoted path: {resolved_output}. "
            "This build must never write to exports/promoted/**."
        )

    week_df = nfl.load_player_stats(seasons=SEASONS, summary_level="week")
    reg_df = nfl.load_player_stats(seasons=SEASONS, summary_level="reg")
    players_df = nfl.load_players()

    payload = build_candidate_payload(week_df, reg_df, players_df)

    OUTPUT_PATH.parent.mkdir(parents=True, exist_ok=True)
    OUTPUT_PATH.write_text(f"{json.dumps(payload, indent=2, allow_nan=False)}\n", encoding="utf-8")
    print(f"Wrote 2021 candidate extension artifact: {OUTPUT_PATH.resolve()}")
