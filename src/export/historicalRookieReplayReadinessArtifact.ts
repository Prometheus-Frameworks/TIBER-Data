...
export function buildHistoricalRookieReplayReadinessV0FromPromotedArtifacts(
  ...
  const rosterByPlayerId = new Map(rosterRows.filter((row) => row.season === replaySeason).map((row) => [row.player_id, row]));
  ...
  const source = [
    ...
    `roster:${options.rosterPath ?? HISTORICAL_ROOKIE_REPLAY_READINESS_V0_SOURCE_ROSTER_PATH}`,
