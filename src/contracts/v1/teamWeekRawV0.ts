...
export function isTeamWeekRawArtifactV0(
  ...
  return teamWeekRawArtifactV0Schema.safeParse(input).success;
}

/**
 * Resolve governance fail-closed. Absent, malformed, or unrecognized governance
 * resolves to ungoverned/not_set. `isGoverned` is true only for an explicit
 * `governed` + `explicit_marker` marker — never inferred from anything else.
 */
export function resolveTeamWeekRawGovernance(
  metadata: { governance?: unknown } | null | undefined,
): {
