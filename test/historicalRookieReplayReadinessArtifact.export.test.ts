...
describe('historical rookie replay readiness v0 artifact export', () => {
  ...
  it('includes committed fixture-backed ready rows', () => {
    const artifact = buildHistoricalRookieReplayReadinessV0FromPromotedArtifacts();
    expect(artifact.some((row) => row.readiness_status === 'ready')).toBe(true);

    const ashton = artifact.find((row) => row.player_name === 'Ashton Jeanty');
    expect(ashton?.readiness_status).toBe('ready');

    const emeka = artifact.find((row) => row.player_name === 'Emeka Egbuka');
    expect(emeka?.readiness_status).toBe('ready');
  });

  it('uses CAR as Tetairoa McMillan expected_team and exposes missing team context', () => {
    const artifact = buildHistoricalRookieReplayReadinessV0FromPromotedArtifacts();
    const tet = artifact.find((row) => row.player_name === 'Tetairoa McMillan');
    expect(tet?.expected_team).toBe('CAR');
    expect(tet?.roster_identity_found).toBe(true);
    expect(tet?.team_pace_pass_found).toBe(false);
    expect(tet?.team_offense_summary_found).toBe(false);
    expect(tet?.missing_evidence).toContain('team_pace_pass_environment');
    expect(tet?.missing_evidence).toContain('team_offense_summary');
    expect(tet?.readiness_status).toBe('missing_team_context');
  });
  ...
  it('uses selected rosterWeek for deterministic identity matching', () => {
    const tempRoot = mkdtempSync(path.join(os.tmpdir(), 'readiness-roster-week-'));
    const replay = JSON.parse(readFileSync(path.resolve(HISTORICAL_ROOKIE_REPLAY_READINESS_V0_SOURCE_REPLAY_PATH), 'utf-8'));
    replay[0].player_id = 'fixture_ashton_jeanty';
    const replayPath = path.join(tempRoot, 'replay-week.json');
    writeFileSync(replayPath, JSON.stringify(replay, null, 2), 'utf-8');

    const roster = JSON.parse(readFileSync(path.resolve(HISTORICAL_ROOKIE_REPLAY_READINESS_V0_SOURCE_ROSTER_PATH), 'utf-8'));
    roster.push({ ...roster.find((row: { player_id: string }) => row.player_id === 'fixture_ashton_jeanty'), week: 2, team: 'ARI' });
    const rosterPath = path.join(tempRoot, 'roster-week.json');
    writeFileSync(rosterPath, JSON.stringify(roster, null, 2), 'utf-8');

    const week1 = buildHistoricalRookieReplayReadinessV0FromPromotedArtifacts({ replayPath, rosterPath, rosterWeek: 1 });
    const week2 = buildHistoricalRookieReplayReadinessV0FromPromotedArtifacts({ replayPath, rosterPath, rosterWeek: 2 });
    const ashtonWeek1 = week1.find((row) => row.player_id === 'fixture_ashton_jeanty');
    const ashtonWeek2 = week2.find((row) => row.player_id === 'fixture_ashton_jeanty');

    expect(ashtonWeek1?.roster_team).toBe('LV');
    expect(ashtonWeek1?.readiness_status).toBe('ready');
    expect(ashtonWeek2?.roster_team).toBe('ARI');
    expect(ashtonWeek2?.readiness_status).toBe('mismatch');
  });
